...
EXPECTED_PRE_HASHES = {
    PRISMA_REL: (
        "84887bcdbe7d4686aa7903b62141f757a896aba59f023ec82403269d8f5eee4d"
    ),
    RC10_REL: (
        "bc8139316d46c4332f0b96e391d1bc5a09740a939e73730b497f324f87b5e52a"
    ),
    AP003G_REL: (
        "9b8883401db7a5b452c9ec8722d3c1d1940ba5f45d1b5d2ac5d98288b1a38f1b"
    ),
}
...
def verify_post(
    ...
    special_entries = [
        ...
        if entry["rc10_consumer_function"] == "main"
    ]

    if len(special_entries) != 1:
        raise SystemExit(
            "O entrypoint especial não é único."
        )

    special = special_entries[0]

    expected_special = {
        "wrapper_name": "run_prisma_generic_entrypoint",
        "candidate_name": "run_prisma_generic_with_runtime",
        "body_function": "_ap003e_body_main_2",
        "rc10_consumer_function": "main",
        "rc10_local_alias": "_ap003e_entrypoint",
    }

    for key, expected_value in expected_special.items():
        if special[key] != expected_value:
            raise SystemExit(
                f"Mapeamento especial divergente em {key}: "
                f"{special[key]!r}"
            )

    special_wrapper = functions[
        special["wrapper_name"]
    ]

    special_candidate = functions[
    ...
    for function in (
        ...
        if (
            ...
            or function.args.vararg.arg != "args"
            or function.args.kwarg is None
            or function.args.kwarg.arg != "kwargs"
        ):
            raise SystemExit(
                f"Assinatura variádica perdida em "
                f"{function.name}."
            )
    ...
    ) != ["run_prisma_generic_with_runtime"]:
        raise SystemExit(
            "Import canônico do entrypoint divergente."
        )
...
def parse_arguments(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Aplicador transacional AP-005B2.4."
        )
    )

...
def render_prisma(
    ...
    if set(candidates) & set(protected):
        raise SystemExit(
            "Um adapter já consta dos nomes protegidos."
        )
...
def apply_transaction(
    ...
    try:
        ...
        if restoration_errors:
            raise RuntimeError(
                "Falha na aplicação e na restauração:\n"
                + "\n".join(
                    restoration_errors
                )
            )
...
def verify_post(
    ...
    if batch_presence != expected_presence:
        raise SystemExit(
            "Estado atômico pós-aplicação divergente: "
            f"{batch_presence}"
        )
    ...
    for entry in payload["entries"]:
        ...
        if batch_name in APPLIED_BATCHES_AFTER:
            ...
            if candidate is None:
                raise SystemExit(
                    f"Adapter ausente: {candidate_name}"
                )
            ...
            if candidate_name not in exports:
                raise SystemExit(
                    f"{candidate_name} ausente de __all__."
                )

            if candidate_name not in protected:
                raise SystemExit(
                    f"{candidate_name} desprotegido."
                )
            ...
            if candidate_name in functions:
                raise SystemExit(
                    f"Adapter futuro aplicado cedo: "
                    f"{candidate_name}"
                )
    ...
    if expected_hashes["orchestrator"] != rc10_hash:
        raise SystemExit(
            "EXPECTED_HASHES['orchestrator'] divergente."
        )
    ...
    if len(selected) != 10:
        raise SystemExit(
            "Seleção AP-005B2.2 divergente."
        )

    print(
        f"PRISMA pós={prisma_hash}"
    )

    print(
        f"RC10 pós={rc10_hash}"
    )

    print(
        f"AP003G pós={ap003g_hash}"
    )
...
def parse_arguments(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Aplicador transacional AP-005B2.2."
        )
    )

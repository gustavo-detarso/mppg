...
# >>> PATCH_PRISMA_ARTIGO_TOML_WRAPPER_V1_4 >>>
def _prisma_artigo_toml_get_arg(argv, name):
...
def _prisma_artigo_toml_strip(argv):
    bool_flags = {"--prisma-exportar-bib", "--prisma-congelar-artigo", "--prisma-gerar-toml-artigo"}
    value_flags = {
        "--prisma-bib-input", "--prisma-bib-output",
        "--prisma-artigo-dir", "--prisma-congelamento-dir",
        "--prisma-artigo-toml-output", "--prisma-csl-path",
        "--prisma-titulo-artigo", "--prisma-autor-artigo",
        "--prisma-professor-artigo", "--prisma-openai-model-artigo",
    }
    result = []
    i = 0
    while i < len(argv):
...
def _prisma_artigo_toml_out_dir(argv):
    from pathlib import Path
    out_arg = (
        _prisma_artigo_toml_get_arg(argv, "--prisma-curadoria-out-dir")
        or _prisma_artigo_toml_get_arg(argv, "--prisma-out-dir")
    )
    if out_arg:
        return Path(out_arg)
    cfg = _prisma_artigo_toml_get_arg(argv, "--config")
    if cfg:
        cfg_path = Path(cfg)
        if cfg_path.exists():
...
def _prisma_artigo_toml_run_export(argv, silent=False):
    import subprocess
    import sys
    from pathlib import Path
    ...
    out_dir = _prisma_artigo_toml_out_dir(argv)
    ...
    bib_input = _prisma_artigo_toml_get_arg(argv, "--prisma-bib-input")
    if bib_input:
        cmd.extend(["--input", bib_input])
    bib_output = _prisma_artigo_toml_get_arg(argv, "--prisma-bib-output")
    if bib_output:
        cmd.extend(["--output", bib_output])
    proc = subprocess.run(cmd)
...
def _prisma_artigo_toml_run_freeze(argv, silent=False):
    import subprocess
    import sys
    from pathlib import Path
    ...
    artigo_dir = _prisma_artigo_toml_get_arg(argv, "--prisma-artigo-dir")
    if artigo_dir:
        cmd.extend(["--artigo-dir", artigo_dir])

    dest_dir = _prisma_artigo_toml_get_arg(argv, "--prisma-congelamento-dir")
    if dest_dir:
        cmd.extend(["--dest-dir", dest_dir])

    if "--prisma-gerar-toml-artigo" in argv:
        cmd.append("--gerar-toml-artigo")

    toml_output = _prisma_artigo_toml_get_arg(argv, "--prisma-artigo-toml-output")
    if toml_output:
        cmd.extend(["--toml-output", toml_output])

    csl_path = _prisma_artigo_toml_get_arg(argv, "--prisma-csl-path")
    if csl_path:
        cmd.extend(["--csl-path", csl_path])

    titulo = _prisma_artigo_toml_get_arg(argv, "--prisma-titulo-artigo")
    if titulo:
        cmd.extend(["--titulo", titulo])

    autor = _prisma_artigo_toml_get_arg(argv, "--prisma-autor-artigo")
    if autor:
        cmd.extend(["--autor", autor])

    professor = _prisma_artigo_toml_get_arg(argv, "--prisma-professor-artigo")
    if professor:
        cmd.extend(["--professor", professor])

    model = _prisma_artigo_toml_get_arg(argv, "--prisma-openai-model-artigo")
    if model:
    ...
    if proc.returncode and not silent:
        raise SystemExit(proc.returncode)
    if proc.returncode and silent:
        print(f"[WARN] Congelamento de insumos retornou código {proc.returncode}.")
    return proc.returncode

_original_main_before_prisma_artigo_toml_wrapper = main

def main(*args, **kwargs):
    import sys
    original_argv = list(sys.argv[1:])
    has_import = (
        "--prisma-curadoria-importar" in original_argv
        or "--prisma-curadoria-fluxo-completo" in original_argv
    )
    ...
    if not has_import and wants_export and not wants_freeze and not wants_toml:
        return _prisma_artigo_toml_run_export(original_argv, silent=False)

    if not has_import and (wants_freeze or wants_toml):
        _prisma_artigo_toml_run_export(original_argv, silent=True)
        return _prisma_artigo_toml_run_freeze(original_argv, silent=False)

    if has_import and (wants_export or wants_freeze or wants_toml):
        old_argv = sys.argv[:]
        sys.argv = [sys.argv[0]] + _prisma_artigo_toml_strip(original_argv)
        try:
            rc = _original_main_before_prisma_artigo_toml_wrapper(*args, **kwargs)
        finally:
            sys.argv = old_argv
    else:
        rc = _original_main_before_prisma_artigo_toml_wrapper(*args, **kwargs)
    ...
    if has_import:
        _prisma_artigo_toml_run_export(original_argv, silent=True)
    if wants_freeze or wants_toml:
        _prisma_artigo_toml_run_freeze(original_argv, silent=False)

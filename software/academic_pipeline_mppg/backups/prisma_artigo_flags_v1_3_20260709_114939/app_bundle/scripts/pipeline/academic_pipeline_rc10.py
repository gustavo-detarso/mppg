...
# >>> PATCH_PRISMA_EXPORTAR_BIB_WRAPPER_V1_2 >>>
def _prisma_bib_wrapper_get_arg(argv, name):
    if name not in argv:
        return None
    idx = argv.index(name)
    if idx + 1 < len(argv):
...
def _prisma_bib_wrapper_out_dir(argv):
    from pathlib import Path
    out_arg = _prisma_bib_wrapper_get_arg(argv, "--prisma-curadoria-out-dir") or _prisma_bib_wrapper_get_arg(argv, "--prisma-out-dir")
    if out_arg:
        return Path(out_arg)
    cfg = _prisma_bib_wrapper_get_arg(argv, "--config")
    if cfg:
        cfg_path = Path(cfg)
        if cfg_path.exists():
...
def _prisma_bib_wrapper_run_export(argv, silent=False):
    import subprocess
    import sys
    from pathlib import Path
    ...
    out_dir = _prisma_bib_wrapper_out_dir(argv)
    ...
    bib_input = _prisma_bib_wrapper_get_arg(argv, "--prisma-bib-input")
    if bib_input:
        cmd.extend(["--input", bib_input])
    bib_output = _prisma_bib_wrapper_get_arg(argv, "--prisma-bib-output")
    if bib_output:
...
_original_main_before_prisma_bib_wrapper = main

def main(*args, **kwargs):
    import sys
    argv = list(sys.argv[1:])
    if "--prisma-exportar-bib" in argv:
        return _prisma_bib_wrapper_run_export(argv, silent=False)

    rc = _original_main_before_prisma_bib_wrapper(*args, **kwargs)

    if "--prisma-curadoria-importar" in argv or "--prisma-curadoria-fluxo-completo" in argv:
        _prisma_bib_wrapper_run_export(argv, silent=True)

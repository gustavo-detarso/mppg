...
def _source_log(provider: str, query_logs: list[dict[str, Any]]) -> dict[str, Any]:
...
def run_external_prisma_search(cfg: dict[str, Any], out_dir: Path, prefix: str, *, progress: Progress = None) -> dict[str, Any]:
    ...
    for source in config["providers"]:
        ...
        for plan in config["query_plans"]:
            ...
            time.sleep(0.20)
        logs.append(_source_log(source, source_queries))
        if source_success:
            successes += 1
    if not successes:
    ...
    _write_json(log_path, {"gerado_em": _now(), "configuracao": search_audit, "fontes": logs, "unpaywall": unpaywall})
    ...
    output = {
        ...
        "fontes": logs,
        "unpaywall": unpaywall,
        "artefatos": {"candidatos_brutos": str(raw_path), "candidatos_deduplicados": str(dedup_path), "planilha_triagem_csv": str(triage_csv), "protocolo": str(protocol), "log": str(log_path)},
    }
    if xlsx:

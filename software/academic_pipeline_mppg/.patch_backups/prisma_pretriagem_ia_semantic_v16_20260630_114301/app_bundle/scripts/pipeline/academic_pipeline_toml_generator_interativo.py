...
STRUCTURED_RESEARCH_TEMPLATE_RELATIVE = Path("misc") / "fgv" / "modelo_relatorio_prisma_busca_orientada.toml"
PROJECT_RESEARCH_MODEL_FILENAME = "dados_pesquisa_prisma.toml"
# Modelo independente para o perfil paper_local_fgv. Ele orienta a redação do
# paper a partir do corpus local, mas nunca substitui os PDFs/DOCX/TXT do ZIP.
STRUCTURED_PAPER_TEMPLATE_RELATIVE = Path("misc") / "fgv" / "modelo_paper_local_fgv.toml"
...
def _structured_research_template_path() -> Path:
    template = (APP / STRUCTURED_RESEARCH_TEMPLATE_RELATIVE).resolve()
    if not template.is_file():
        raise ValueError(
            "O modelo estruturado neutro não foi encontrado. Esperado em: "
            f"{template}"
        )
    return template
...
def ask_structured_research_source(
    ...
    *,
    ...
    """Escolhe arquivo existente ou uma cópia editável do modelo do projeto."""
    while True:
        choice = ask_choice(
            "Como deseja fornecer o arquivo estruturado",
            [
                "usar arquivo já existente",
                "copiar e editar o modelo padrão na pasta do projeto",
            ],
            "usar arquivo já existente",
        )
        if choice == "usar arquivo já existente":
            raw_path, values = ask_structured_research_file(prompt, required_fields=required_fields)
            return raw_path, values, False

        try:
            template = _structured_research_template_path()
        ...
        reviewed = review_imported_research_values(
            str(destination),
            values,
            fields_to_edit,
            default_edit=True,
...
def _structured_paper_template_path() -> Path:
    template = (APP / STRUCTURED_PAPER_TEMPLATE_RELATIVE).resolve()
    if not template.is_file():
        raise ValueError(
            "O modelo estruturado de paper não foi encontrado. Esperado em: "
            f"{template}"
        )
    return template
...
def ask_structured_paper_source(
    ...
    *,
    ...
    while True:
        ...
        try:
            template = _structured_paper_template_path()
        ...
        reviewed = review_imported_paper_values(
            str(destination),
            values,
            editable_fields,
            default_edit=True,
...
def collect_prisma_busca_orientada(data: dict[str, Any]) -> None:
    ...
    print(
        ...
        "A busca externa será executada antes da triagem; o sistema não decidirá inclusão ou exclusão de estudos."
    )
    ...
    if mode == "arquivo estruturado":
        ...
        data["fonte_dados_pesquisa"] = "arquivo_estruturado"
    ...
    data["criterios_inclusao"] = ask_list(
    ...
    data["criterios_exclusao"] = ask_list(

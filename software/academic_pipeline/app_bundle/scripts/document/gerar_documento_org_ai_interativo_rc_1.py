...
# -*- coding: utf-8 -*-
"""
Gerador autocontido de documento acadêmico em Org-mode a partir de template .org,
com suporte a textos-base, orientações externas, .zip de artigos, inferência via OpenAI,
...
3. Infere primeiro um contexto-base amplo e, em seguida, afunila o documento em segunda etapa.
...
DEFAULT_TEMPLATE_ORG = "template_documento.org"
...
def configure_keywords_and_queries(client: OpenAI, model: str, context: DocumentContext, bases: list[str], tipo_estudo: str, idiomas: list[str], prompt_log: list[tuple[str, str]]) -> tuple[list[str], dict[str, str | None]]:
...
def rank_correlated_candidates(client: OpenAI, model: str, context: DocumentContext, base_docs: list[SourceDoc], candidates: list[CandidatePaper], max_items: int, prompt_log: list[tuple[str, str]]) -> RankedReferenceOutput:
    payload = [
        {
            "paper_id": c.paper_id,
            "title": c.title,
...
def run_related_search_flow(client: OpenAI, model: str, context: DocumentContext, output_dir: Path, basename: str, prompt_log: list[tuple[str, str]]) -> tuple[list[SourceDoc], dict[str, Any]]:
...
def infer_final_front_matter(client: OpenAI, model: str, context: DocumentContext, org_text: str, prompt_log: list[tuple[str, str]]) -> FinalFrontMatterOutput:
    prompt = render_prompt_file(
...
def generate_paper_org(client: OpenAI, model: str, template_text: str, context: DocumentContext, base_docs: list[SourceDoc], guidance_docs: list[SourceDoc], correlated_docs: list[SourceDoc], bib_filename: str, style: str, prompt_log: list[tuple[str, str]]) -> str:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Gerador autocontido de documento acadêmico em Org-mode com IA e busca correlata opcional.")
...
def main() -> int:
    ...
    template_path = prompt_path("Template .org do documento acadêmico (indique apenas um arquivo .org)", template_default, must_exist=True)
    if template_path.suffix.lower() != ".org":
        raise RuntimeError(f"O template do documento acadêmico deve ser um arquivo .org. Recebido: {template_path}")

...
Versão v0.2.4: inclui enriquecimento bibliográfico de documentos locais por bases externas.
...
def query_crossref_metadata(meta: InferredBibMetadata, doc: SourceDoc, cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    headers = {"User-Agent": f"academic-pipeline/0.2.4 ({mailto or 'local-use'})"}
...
def query_semantic_scholar_metadata(meta: InferredBibMetadata, doc: SourceDoc, cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    headers = {"User-Agent": "academic-pipeline/0.2.4"}
    if key:
...
def query_scopus_metadata(meta: InferredBibMetadata, doc: SourceDoc, cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not key:
        return []
    headers = {"X-ELS-APIKey": key, "Accept": "application/json", "User-Agent": "academic-pipeline/0.2.4"}
    candidates: list[dict[str, Any]] = []
    doi = _clean_doi(meta.doi) or _extract_doi_from_text(doc.extracted_text)
    if doi:
        query = f"DOI({doi})"
...
def build_revised_bib_entries_for_local_docs(
    ...
    for doc in docs:
        ...
        if local_metadata_enrichment_enabled(cfg) and bibliographic_metadata_incomplete(meta, cfg):

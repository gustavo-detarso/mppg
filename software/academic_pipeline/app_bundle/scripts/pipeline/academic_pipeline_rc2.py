...
def infer_bib_metadata_for_doc(client: OpenAI, model: str, doc: SourceDoc, citation_style: str | None = None) -> InferredBibMetadata:
    ...
    prompt = textwrap.dedent(
        ...
        - para metadados inferidos com baixa certeza, use note = "Metadados bibliográficos inferidos automaticamente; revisar manualmente.";
...
def build_revised_bib_entries_for_local_docs(
    ...
    Diferente do .bib sintético, esta rotina lê o texto extraído dos PDFs/DOCX
    locais e tenta identificar tipo de obra, autores, título, ano, periódico,
    livro/coletânea, editora, páginas, DOI/URL etc. Quando a IA não consegue
    inferir com segurança, cai em @misc com nota de revisão manual.
    """
    ...
    for doc in docs:
        try:
            ...
            meta = InferredBibMetadata(
                ...
                year=str(local.get("ano_padrao") or "s.d."),
                note="Metadados bibliográficos inferidos automaticamente; revisar manualmente.",
            )
        ...
        if not meta.authors and not meta.editors:
            meta.authors = [str(local.get("autor_padrao") or "Material fornecido pelo professor")]
        if not meta.year:
            meta.year = str(local.get("ano_padrao") or "s.d.")
        if not meta.note and source != "ia":
            meta.note = "Metadados bibliográficos inferidos automaticamente; revisar manualmente."
...
def maybe_generate_revised_local_bib(
    ...
    write_text(bib_path, "\n\n".join((existing_entries if existing_entries else []) + revised_entries).strip() + "\n")
    revised_sidecar = bib_path.with_name(bib_path.stem + "_revisado_ia.bib")
    write_text(revised_sidecar, "\n\n".join(revised_entries).strip() + "\n")
...
def remove_existing_mindmap_sections(org_text: str) -> str:
    """Remove somente blocos automáticos/artefatos de mapa mental.

    Versão segura para re-renderização:
    - remove blocos delimitados por MINDMAP_AUTO_START/MINDMAP_AUTO_END;
    - remove blocos LaTeX antigos que incluam mapa_mental.png/svg;
    - remove blocos PlantUML crus ou #+begin_src plantuml contendo mindmap;
    - remove comentários antigos com código PlantUML.

    Intencionalmente NÃO remove headings contendo "mapa mental". A versão
    anterior fazia uma remoção por heading e podia apagar trechos legítimos do
    documento quando o mapa antigo era interpretado como seção Org.
    """
    text = org_text.replace("\r\n", "\n")
    ...
    # 2) Remove blocos LaTeX antigos do mapa mental gerados antes da marcação.
    # Restrito ao includegraphics de arquivo chamado mapa_mental.png/svg.
    text = re.sub(
    ...
    # 3) Remove blocos src PlantUML somente quando contêm @startmindmap.
    text = re.sub(
    ...
    # 4) Remove trechos crus @startmindmap...@endmindmap que tenham vazado.
    text = re.sub(r"(?ims)^\s*@startmindmap\b.*?@endmindmap\s*$\n?", "\n\n", text)
    text = re.sub(r"(?im)^\s*Arquivo PlantUML gerado:.*$\n?", "", text)

    # 5) Remove comentários antigos contendo o PlantUML.
    text = re.sub(
...
def insert_mindmap_section(
    ...
    *,
    ...
    """Insere apenas a imagem renderizada do mapa mental.

    O .org não recebe bloco `#+begin_src plantuml` por padrão. Isso evita que
    o código seja exportado como texto quando o bloco não é avaliado. Para
    garantir a posição APÓS as referências no PDF, a inserção é feita depois de
    #+PRINT_BIBLIOGRAPHY: e usa LaTeX cru não flutuante com \includegraphics.
    """
    title = title.strip() or "Mapa mental dos textos analisados"
    org_text = remove_existing_mindmap_sections(org_text)

    # Sem imagem renderizada, não insere nada no .org. O .puml continua salvo
    # para depuração/renderização manual.
    ...
    latex_lines = [
        ...
        r"\begingroup",
        r"\centering",
        r"{\Large\bfseries " + safe_title + r"\par}",
        r"\vspace{1em}",
        r"\includegraphics[width=\textwidth,height=0.86\textheight,keepaspectratio]{" + safe_image + r"}",
        r"\par",
        r"\endgroup",
        r"\clearpage",
        "#+end_export",
    ...
    if pos in {"antes_consideracoes", "antes_conclusao", "apos_sintese", "depois_sintese"}:
        m = re.search(r"(?im)^\*+\s+(Considera[cç][oõ]es finais|Conclus[aã]o|Conclus[oõ]es)\s*$", org_text)
        if m:
            return org_text[:m.start()].rstrip() + "\n\n" + latex_block + org_text[m.start():].lstrip("\n")

    # Padrão: depois da diretiva bibliográfica. No export, a diretiva gera as
    # referências exatamente naquele ponto; o bloco posterior sai após elas.
    m = re.search(r"(?im)^\s*#\+PRINT_BIBLIOGRAPHY:?\s*$", org_text)
    if m:
        line_end = org_text.find("\n", m.end())
        if line_end == -1:
            line_end = m.end()
        return org_text[:line_end].rstrip() + "\n\n" + latex_block + org_text[line_end:].lstrip("\n")

    # Se não houver bibliografia no ORG, recria a diretiva e coloca o mapa em
    # seguida, preservando a ordem: texto -> referências -> mapa.
...
def main() -> int:
    ...
    if normalize_document_type(documento.get("tipo_documento")) != "dissertacao" and configured_title:

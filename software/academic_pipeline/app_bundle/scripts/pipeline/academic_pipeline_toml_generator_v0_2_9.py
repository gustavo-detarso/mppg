...
PROFILE_INFO: dict[str, dict[str, str]] = {
    "atividade_local": {
        ...
        "fluxo": "Lê ZIP ou pasta local, monta corpus local, gera .bib revisado com DOI/CSV quando houver, enriquece metadados em bases externas, cria .org acadêmico e exporta PDF.",
...
ORIENTATION_READABLE_SUFFIXES = {".txt", ".md", ".org", ".rst", ".tex", ".json", ".csv", ".yaml", ".yml", ".xml", ".pdf", ".docx"}
...
def section_atividade(values: dict[str, Any] | None = None) -> str:
    ...
    curso = {toml_quote(v.get('curso', 'Mestrado Acadêmico em Políticas Públicas e Governo'))}
...
def section_documentos_locais(values: dict[str, Any] | None = None) -> str:
    ...
    # Bibliografia local:
    # - gerar_bib_revisado_ia=true usa IA para extrair metadados dos PDFs/DOCX locais;
    # - enriquecer_metadados_buscadores=true consulta bases externas apenas para corrigir
    #   metadados incompletos dos documentos locais, sem executar fluxo PRISMA.
    gerar_bib_revisado_ia = {toml_bool(v.get('gerar_bib_revisado_ia', True))}
    max_docs_bib_revisado_ia = {int(v.get('max_docs_bib_revisado_ia', 30))}
    max_caracteres_bib_revisado = {int(v.get('max_caracteres_bib_revisado', 35000))}

    enriquecer_metadados_buscadores = {toml_bool(v.get('enriquecer_metadados_buscadores', True))}
    fontes_metadados = {toml_array(v.get('fontes_metadados', ['crossref', 'openalex', 'semantic_scholar', 'scopus']))}
    min_score_match_metadados = {float(v.get('min_score_match_metadados', 0.82))}

    # DOI manual/automático para metadados mais confiáveis.
    # CSV esperado: arquivo,doi
    # Use também em atividades: se o PDF/DOCX não permitir inferir a referência, o DOI manual será priorizado.
    extrair_doi_dos_pdfs = true
    doi_manifest_path = {toml_quote(v.get('doi_manifest_path', ''))}
    preferir_doi_manual = true
    buscar_metadados_por_doi = true
    falhar_se_doi_manual_nao_encontrado = false

    preferir_doi = true
    salvar_diagnostico_metadados = true
    incluir_notas_metadados_inferidos = false
    deduplicar_bib = true
    deduplicar_referencias = true

    # Fallback discreto: evite "Material fornecido pelo professor", pois isso vaza na referência.
    autor_padrao = {toml_quote(v.get('autor_padrao', ''))}
    ano_padrao = {toml_quote(v.get('ano_padrao', 's.d.'))}
    """).strip() + "\n\n"
...
def section_documento(values: dict[str, Any] | None = None) -> str:
    ...
    # Template Org opcional. Informe preferencialmente template_org.
    # template_path é mantido por compatibilidade com versões anteriores.
    template_org = {toml_quote(v.get('template_org', v.get('template_path', '')))}
    template_path = {toml_quote(v.get('template_path', v.get('template_org', '')))}

    titulo_trabalho = {toml_quote(v.get('titulo_trabalho', 'Atividade Aula 2'))}

    # Se tema/recorte/objetivo/título vierem vazios ou genéricos, o pipeline
    # pode inferi-los via IA a partir do corpus local/pesquisa e das orientações.
    inferir_campos_vazios_ia = {toml_bool(v.get('inferir_campos_vazios_ia', True))}

    estilo_citacao = {toml_quote(v.get('estilo_citacao', 'apa'))}

    # Metadados determinísticos de capa para fgv-paper.
    # Em paper, program_name deve ficar vazio para não duplicar o curso na capa.
    institution_name = {toml_quote(v.get('institution_name', 'Fundação Getúlio Vargas'))}
    program_name = {toml_quote(v.get('program_name', ''))}
    course_name = {toml_quote(v.get('course_name', 'Mestrado Acadêmico em Políticas Públicas e Governo'))}
    discipline_name = {toml_quote(v.get('discipline_name', v.get('disciplina', '')))}
    professor_name = {toml_quote(v.get('professor_name', v.get('professor', '')))}
    city_name = {toml_quote(v.get('city_name', 'Brasília'))}
    papertype = {toml_quote(v.get('papertype', 'Paper acadêmico'))}
    covernote = {toml_quote(v.get('covernote', 'Trabalho acadêmico elaborado para a disciplina.'))}

    # Blindagens finais do ORG.
    usar_citacoes_latex_diretas = {toml_bool(v.get('usar_citacoes_latex_diretas', True))}
    normalizar_capa_paper = {toml_bool(v.get('normalizar_capa_paper', True))}
    remover_mencoes_tecnicas_pipeline = {toml_bool(v.get('remover_mencoes_tecnicas_pipeline', True))}
    corrigir_citacoes_narrativas = {toml_bool(v.get('corrigir_citacoes_narrativas', True))}
    validar_org_final = {toml_bool(v.get('validar_org_final', True))}
    falhar_se_org_tiver_chave_crua = {toml_bool(v.get('falhar_se_org_tiver_chave_crua', True))}
    falhar_se_org_tiver_empty_citation = {toml_bool(v.get('falhar_se_org_tiver_empty_citation', True))}
    falhar_se_org_tiver_mencao_tecnica = {toml_bool(v.get('falhar_se_org_tiver_mencao_tecnica', True))}
    ...
    # Orientações específicas do documento. Pode conter arquivos, pastas ou ZIPs.
    # Se houver versões preliminar/final no mesmo ZIP, o pipeline prioriza arquivos com "final" no nome, sem descartar os demais.
    orientacoes_paths = {toml_array(v.get('orientacoes_paths', []))}
    orientacao_inline = {toml_quote(v.get('orientacao_inline', ''))}
...
def section_atividade_excelencia(values: dict[str, Any] | None = None) -> str:
    v = values or {}
    curso = v.get('curso', 'Mestrado Acadêmico em Políticas Públicas e Governo')
...
def section_documento_dissertacao_excelencia(values: dict[str, Any] | None = None) -> str:
    ...
    # Se título/tema/recorte/objetivo estiverem vazios ou genéricos, inferir via IA
    # com base na pesquisa/corpus local e nas orientações.
    inferir_campos_vazios_ia = {toml_bool(v.get('inferir_campos_vazios_ia', True))}
...
def interactive_values(profile: str) -> dict[str, dict[str, Any]]:
    ...
    if profile in {"atividade_local", "pesquisa_prisma_documento", "pesquisa_dissertacao_excelencia", "paper", "dissertacao", "derivacao_dissertacao"}:
        ...
        values["atividade"] = {
            "curso": ask_text("Curso", "Mestrado Acadêmico em Políticas Públicas e Governo"),
    ...
    if profile == "atividade_local":
        ...
        local.update({
            ...
            "enriquecer_metadados_buscadores": True,
            "fontes_metadados": ["crossref", "openalex", "semantic_scholar", "scopus"],
            "extrair_doi_dos_pdfs": True,
            "preferir_doi_manual": True,
            "buscar_metadados_por_doi": True,
            "deduplicar_bib": True,
        ...
        if ask_bool("Usar/gerar CSV de DOI para esta atividade?", True):
            local["doi_manifest_path"] = ask_path("Caminho do doi_manifest.csv", "./doi_manifest.csv")
        else:
            local["doi_manifest_path"] = ""
        ...
        values["documento"] = {
            ...
            "template_org": ask_path("Template .org opcional", ""),
...
def validate_orientation_source(label: str, raw: Any, base_dir: Path, errors: list[str], warnings: list[str]) -> None:
    if not is_nonempty(raw):
        return
    p = resolve_path(str(raw), base_dir)
    if not p.exists():
    ...
    if p.is_dir():
        return
    if not p.is_file():
        errors.append(f"{label}: esperado arquivo, diretório ou ZIP: {p}")
        return
    suffix = p.suffix.lower()
    if suffix not in ORIENTATION_READABLE_SUFFIXES and suffix not in ORIENTATION_ARCHIVE_SUFFIXES:
        warnings.append(f"{label}: extensão não suportada como orientação ({suffix}): {p}")
...
def validate_config(path: Path) -> tuple[list[str], list[str]]:
    ...
    if documento:
        validate_path("[documento].template_org", documento.get("template_org", ""), base_dir, errors, warnings, must_be_file=True)
        ...
        for raw in iter_paths_in_value(documento.get("orientacoes_paths", [])):
            validate_orientation_source("[documento].orientacoes_paths", raw, base_dir, errors, warnings)
    ...
    if deriv.get("ativo"):
        ...
        for raw in iter_paths_in_value(deriv.get("orientacoes_paths", [])):
            validate_orientation_source("[derivacao].orientacoes_paths", raw, base_dir, errors, warnings)

...
def call_module_main(module_name: str, argv: list[str], quiet: bool = False) -> str:
    ...
    try:
        ...
        stdout_cm = contextlib.redirect_stdout(buf) if quiet else contextlib.nullcontext()
        stderr_cm = contextlib.redirect_stderr(buf) if quiet else contextlib.nullcontext()
        ...
        if code != 0:
            raise RuntimeError(
                f"Falha ao executar {module_name} com código {code}.\n"
                f"Saída capturada:\n{captured[-4000:]}"
            )
        return captured
    finally:
...
def ensure_cfg(cfg: Path, template_cfg: Path | None, quiet: bool = False) -> None:
    ...
    if template_cfg is None:
        raise SystemExit(
            f"ERRO: TOML não encontrado: {cfg}\n"
...
def set_toml_value_if_present(txt: str, key: str, value: str) -> str:
    """Atualiza chave simples existente no TOML sem risco de backreference regex."""
...
def sanitize_bib(bib: Path, quiet: bool = False) -> None:
    ...
    field_names = [
        "author", "editor", "title", "journaltitle", "journal", "publisher",
...
def project_root() -> Path:
    """Retorna a raiz provável do projeto, mesmo quando este arquivo está em app_bundle/scripts/pipeline."""
    here = Path(__file__).resolve()
    candidates = [Path.cwd().resolve()] + list(here.parents)
    for cand in candidates:
        if (cand / "app_bundle").exists() and ((cand / "Pipfile").exists() or (cand / "pyproject.toml").exists()):
            return cand
...
def export_org_to_tex(org: Path, out: Path, quiet: bool = False) -> None:
    ...
    if academic_el.exists():
        eval_lisp = (
            "(progn "
            "(require (quote org)) "
            "(require (quote ox-latex)) "
            f"(load-file {str(academic_el)!r}) "
            "(setq org-confirm-babel-evaluate nil) "
            f"(find-file {str(org)!r}) "
            "(org-latex-export-to-latex))"
        )
    else:
        # Fallback autossuficiente para a classe Org fgv-paper. O arquivo LaTeX real é fgv-paper.sty.
        eval_lisp = (
            "(progn "
            "(require (quote org)) "
            "(require (quote ox-latex)) "
            "(add-to-list (quote org-latex-classes) "
            "(quote (\"fgv-paper\" "
    ...
    code, output = run(["emacs", "--batch", "--eval", eval_lisp], cwd=out, quiet=quiet)
    if code != 0:
...
def insert_tex_bib_config(tex: str, bib_name: str) -> str:
    ...
    insert = (
        rf"\usepackage[{BIBLATEX_ABNT_OPTIONS}]{{biblatex}}" + "\n" +
...
def compile_pdf(out: Path, prefix: str, quiet: bool = False) -> Path:
    ...
    tex_paths = [
        Path("."),
        out,
        root / "app_bundle" / "misc" / "fgv",
        root / "app_bundle" / "institutions" / "fgv" / "latex",
    ]
    tex_inputs = ":".join(str(p) for p in tex_paths if p.exists() or str(p) == ".") + ":"
    ...
    if not bbl.exists() or "\\entry" not in bbl.read_text(encoding="utf-8", errors="ignore"):
        raise SystemExit(f"ERRO: Biber não gerou entradas válidas em {bbl}")
...
def validate_pdf(pdf: Path, quiet: bool = False) -> None:
    ...
    bad_patterns = [
        r"LATEX", r"LaTeX", r"\[[0-9]+(?:,\s*[0-9]+)*\]",
        r"Soares, Ninin e Lima \(Soares", r"Gabbay et al\. \(Gabbay", r"Dijkstra \(Dijkstra",
    ]
    hits: list[str] = []
    ...
    if hits:
        raise SystemExit(
            "ERRO: validação encontrou problemas no PDF final:\n" +
            "\n".join(hits[:40])
        )
    if not quiet:
...
def prepare_fulltext_stage(art: Path, cfg: Path, target_n: int, min_palavras: int, chars_por_pdf: int, quiet: bool = False) -> None:
    ...
    call_module_main(
        ...
        [
            ...
            "--min-palavras", str(min_palavras),
            "--chars-por-pdf", str(chars_por_pdf),
        ],
        quiet=quiet,
    )
...
def final_fix_stage(art: Path, prefix: str, quiet: bool = False) -> Path:
    out = art / "output"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Gera artigo final em PDF: preparação, geração, ABNT/Biber, compilação e validação."
    )
    parser.add_argument("--art-dir", required=True, help="Diretório do artigo.")
    parser.add_argument("--cfg-art", required=True, help="Arquivo TOML do artigo.")
    parser.add_argument("--template-cfg", default=None, help="Template TOML, usado apenas se --cfg-art não existir.")
    parser.add_argument("--prefix", default=None, help="Prefixo dos arquivos de saída. Padrão: stem do TOML.")
    ...
    parser.add_argument("--skip-prepare", action="store_true", help="Pula preparação do corpus full text.")
    parser.add_argument("--skip-generate", action="store_true", help="Pula geração textual e apenas finaliza arquivos existentes.")
    parser.add_argument("--quiet", action="store_true", help="Mostra apenas resultado final ou erro.")
    args = parser.parse_args()

    art = Path(args.art_dir).expanduser().resolve()
    cfg = Path(args.cfg_art).expanduser().resolve()
    template_cfg = Path(args.template_cfg).expanduser().resolve() if args.template_cfg else None
    ...
    recompilacao_apenas = bool(args.skip_prepare and args.skip_generate)
    if not recompilacao_apenas:
    ...
    pdf = final_fix_stage(art, prefix, quiet=args.quiet)
...
def main() -> int:
    ...
    for target in targets:
        ...
        if target.exists():
            bak = target.with_name(target.name + f".bak_fix_v3_{stamp}")
    ...
    print("[OK] py_compile passou para os dois geradores.")
    return 0

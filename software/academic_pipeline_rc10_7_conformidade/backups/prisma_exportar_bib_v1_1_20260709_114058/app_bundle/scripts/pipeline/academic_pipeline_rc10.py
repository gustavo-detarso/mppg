...
# >>> PATCH_PRISMA_EXPORTAR_BIB_V1 >>>
def _prisma_exportar_bib_default_out_dir(args):
    from pathlib import Path
    out_arg = getattr(args, "prisma_curadoria_out_dir", None) or getattr(args, "prisma_out_dir", None)
    if out_arg:
        return Path(out_arg)
...
def _prisma_exportar_bib_cli(args, silent=False):
    import subprocess, sys
    from pathlib import Path
    helper = Path(__file__).with_name("prisma_exportar_bib.py")
    if not helper.exists():
        msg = f"Helper de exportação BibLaTeX não encontrado: {helper}"
        print(f"[WARN] {msg}" if silent else msg)
        if silent:
            return 1
        raise SystemExit(msg)
    out_dir = _prisma_exportar_bib_default_out_dir(args)
    ...
    bib_input = getattr(args, "prisma_bib_input", None)
    if bib_input:
        cmd += ["--input", str(bib_input)]
    bib_output = getattr(args, "prisma_bib_output", None)
    if bib_output:
        cmd += ["--output", str(bib_output)]
    proc = subprocess.run(cmd)
...
def main() -> int:
    ...
    parser.add_argument(\n        "--prisma-exportar-bib",\n        action="store_true",\n        help="Exporta arquivo .bib/.biblatex a partir das referências incluídas no PRISMA.",\n    )\n    parser.add_argument(\n        "--prisma-bib-input",\n        default=None,\n        help="CSV de entrada para --prisma-exportar-bib.",\n    )\n    parser.add_argument(\n        "--prisma-bib-output",\n        default=None,\n        help="Arquivo .bib de saída para --prisma-exportar-bib.",\n    )\n
    parser.add_argument(
    ...
    args = parser.parse_args()
        ...
        if getattr(args, "prisma_exportar_bib", False):\n        return _prisma_exportar_bib_cli(args, silent=False)\n\n    if getattr(args, "prisma_curadoria_importar", False) or getattr(args, "prisma_curadoria_fluxo_completo", False):\n        import atexit\n        atexit.register(lambda: _prisma_exportar_bib_cli(args, silent=True))\n

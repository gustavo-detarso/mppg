...
def draw_diagram_png(counts: dict[str, int], png_path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(8.2, 10.5), dpi=200)
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 14)
    ax.axis("off")
    ...
    ax.text(5, 13.55, "Fluxo PRISMA adaptado", ha="center", va="center", fontsize=14, fontweight="bold")
    ax.text(5, 13.18, "Busca, deduplicação, pré-triagem por IA, triagem humana e inclusão final",
            ha="center", va="center", fontsize=8.5)

    # Coluna principal
    box(2.2, 12.1, 5.6, 0.75, "Registros identificados nas bases", f"n = {identified}", face="#EAF2F8")
    box(2.2, 10.75, 5.6, 0.75, "Registros após deduplicação", f"n = {dedup}", face="#EAF2F8")
    box(2.2, 9.4, 5.6, 0.75, "Registros avaliados na pré-triagem IA", f"n = {preai}", face="#EEF7EE")
    box(2.2, 8.05, 5.6, 0.75, "Registros enviados à triagem humana", f"n = {sent}", face="#EEF7EE")
    box(2.2, 6.7, 5.6, 0.75, "Textos completos/registros avaliados para inclusão", f"n = {full}", face="#FFF7E6")
    box(2.2, 5.35, 5.6, 0.75, "Estudos incluídos na revisão", f"n = {inc}", face="#E9F7EF", edge="#1E8449")

    # Caixas laterais de exclusão
    box(7.95, 10.75, 1.85, 0.75, "Duplicatas removidas", f"n = {dup}", fontsize=7.3, face="#FDEDEC", edge="#922B21")
    box(7.95, 9.4, 1.85, 0.75, "Não avaliados na pré-triagem IA", f"n = {dedup_not_preai}", fontsize=7.3, face="#FDEDEC", edge="#922B21")
    box(7.95, 8.05, 1.85, 0.75, "Não enviados à triagem humana", f"n = {preai_not_sent}", fontsize=7.3, face="#FDEDEC", edge="#922B21")
    box(7.95, 6.7, 1.85, 0.75, "Excluídos em título/resumo", f"n = {title_exc}", fontsize=7.3, face="#FDEDEC", edge="#922B21")
    box(7.95, 5.35, 1.85, 0.75, "Excluídos após texto completo", f"n = {full_exc}", fontsize=7.3, face="#FDEDEC", edge="#922B21")
    ...
    # Setas laterais
    arrow(7.8, 11.15, 7.95, 11.15)
    arrow(7.8, 9.78, 7.95, 9.78)
    arrow(7.8, 8.43, 7.95, 8.43)
    arrow(7.8, 7.08, 7.95, 7.08)
    arrow(7.8, 5.73, 7.95, 5.73)

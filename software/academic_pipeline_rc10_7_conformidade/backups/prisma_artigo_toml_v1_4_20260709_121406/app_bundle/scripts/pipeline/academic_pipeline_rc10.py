...
# >>> PATCH_PRISMA_ARTIGO_FLAGS_WRAPPER_V1_3 >>>
def _prisma_artigo_extra_get_arg(argv, name):
...
def _prisma_artigo_extra_strip(argv):
    bool_flags = {"--prisma-exportar-bib", "--prisma-congelar-artigo"}
    value_flags = {"--prisma-bib-input", "--prisma-bib-output", "--prisma-artigo-dir", "--prisma-congelamento-dir"}
    result = []
    i = 0
    while i < len(argv):
        item = argv[i]
...
def _prisma_artigo_extra_out_dir(argv):
    from pathlib import Path
    out_arg = (
        _prisma_artigo_extra_get_arg(argv, "--prisma-curadoria-out-dir")
        or _prisma_artigo_extra_get_arg(argv, "--prisma-out-dir")
    )
    if out_arg:
        return Path(out_arg)
    cfg = _prisma_artigo_extra_get_arg(argv, "--config")
    if cfg:
        cfg_path = Path(cfg)
        if cfg_path.exists():
...
def _prisma_artigo_extra_run_export(argv, silent=False):
    import subprocess
    import sys
    from pathlib import Path
    ...
    out_dir = _prisma_artigo_extra_out_dir(argv)
    ...
    bib_input = _prisma_artigo_extra_get_arg(argv, "--prisma-bib-input")
    if bib_input:
        cmd.extend(["--input", bib_input])
    bib_output = _prisma_artigo_extra_get_arg(argv, "--prisma-bib-output")
    if bib_output:
...
def _prisma_artigo_extra_run_freeze(argv, silent=False):
    import subprocess
    import sys
    from pathlib import Path
    ...
    prefix = out_dir.name if out_dir.name.startswith("relatorio_prisma_") else "relatorio_prisma_prisma_fluxo_pmf"
    cmd = [sys.executable, str(helper), "--out-dir", str(out_dir), "--prefix", prefix]
    artigo_dir = _prisma_artigo_extra_get_arg(argv, "--prisma-artigo-dir")
    if artigo_dir:
        cmd.extend(["--artigo-dir", artigo_dir])
    dest_dir = _prisma_artigo_extra_get_arg(argv, "--prisma-congelamento-dir")
    if dest_dir:
...
_original_main_before_prisma_artigo_flags_wrapper = main
...
def main(*args, **kwargs):
    ...
    if not has_import and wants_export and not wants_freeze:
        return _prisma_artigo_extra_run_export(original_argv, silent=False)

    if not has_import and wants_freeze:
        if not wants_export:
            _prisma_artigo_extra_run_export(original_argv, silent=True)
        else:
            _prisma_artigo_extra_run_export(original_argv, silent=False)
        return _prisma_artigo_extra_run_freeze(original_argv, silent=False)

    if has_import and (wants_export or wants_freeze):
        old_argv = sys.argv[:]
        sys.argv = [sys.argv[0]] + _prisma_artigo_extra_strip(original_argv)
        try:
            rc = _original_main_before_prisma_artigo_flags_wrapper(*args, **kwargs)
        finally:
            sys.argv = old_argv
    else:
        rc = _original_main_before_prisma_artigo_flags_wrapper(*args, **kwargs)
    ...
    if has_import:
        _prisma_artigo_extra_run_export(original_argv, silent=True)
    if wants_freeze:
        _prisma_artigo_extra_run_freeze(original_argv, silent=False)
